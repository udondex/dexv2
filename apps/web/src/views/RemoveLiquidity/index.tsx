  ...
  async function onRemove() {
    ...
    for (let i = 0; i < methodNames.length; i++) {
      ...
      try {
        ...
        const err = e as { error: { data: { code: number; message: string } } }
        if (err.error.data.code === 3) {
          toastError(
            t('Error'),
            <>
              <Text>{t('KYC is required')}</Text>
              <Text>{t('Please verify your account to continue')}</Text>
              <Button mt="10px" onClick={() => window.open('https://kkub-otp.bitkubchain.com', '_blank')}>
